...
open class ColorCircle: UIControl {
    ...
    open override func layoutSubviews() {
        ...
        setColor(rgb.color, animated: false, sendEvent: false)
    }
    ...
    open override func beginTracking(_ touch: UITouch, with event: UIEvent?) -> Bool {
        ...
        let hsb = HSB(newHue, newSaturation, rgb.hsb.brightness, rgb.hsb.alpha)
        setColor(hsb.color, animated: false, sendEvent: true)
    ...
    open func setColor(_ color: UIColor, animated: Bool, sendEvent: Bool) {
        ...
        if brightnessLayer.alpha != 1 - rgb.hsb.brightness {
            brightnessLayer.alpha = 1 - rgb.hsb.brightness
        }
        
        // Hue = point angle / 2*Pi, saturation = distance to point, in palette's coordinate system with origin in the center
        let newCursorCenter = CGPoint(x:  cos(rgb.hsb.hue * 2 * .pi) * (rgb.hsb.saturation * paletteRadius) + paletteRadius,
                                      y: -sin(rgb.hsb.hue * 2 * .pi) * (rgb.hsb.saturation * paletteRadius) + paletteRadius)
